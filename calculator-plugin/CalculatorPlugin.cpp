...
void CalculatorPlugin::initialize() {
    logDir = QStandardPaths::writableLocation(QStandardPaths::AppDataLocation);
    QDir().mkpath(logDir + "/plugins");  // Make sure the directory exists
    m_logPath = logDir + "/plugins/calcplugin.log";

    confDir = QStandardPaths::writableLocation(QStandardPaths::AppConfigLocation) + "/plugins";
    QDir().mkpath(confDir);  // Make sure the directory exists

    QString filePath = confDir + "/calcplugin.ini";

    if (!QFile::exists(filePath)) {
        QFile file(filePath);
        if (file.open(QIODevice::WriteOnly)) {
            file.close();
            qDebug() << "Created new settings file:" << filePath;
        }
        QSettings settings(filePath, QSettings::IniFormat);
        settings.beginGroup("User");
        settings.setValue("username", "user123");
        settings.endGroup();
        Utils::logMessage(m_logPath,"Calculator plugin default configuration created!");
    }
    else
    {
        QSettings settings(filePath, QSettings::IniFormat);
        ...
        settings.endGroup();
        Utils::logMessage(m_logPath,"Calculator plugin configuration loaded! Username is " + username);
    }

    Utils::logMessage(m_logPath,"Calculator plugin initialized");
}
...
QWidget* CalculatorPlugin::pcontent()
{
    ...
    test->setWindowIcon(QIcon(":/resources/icon.png"));
